...
const OBSERVE_TIMEOUT_MS = Number(process.env.CODEX_BG_OBSERVE_TIMEOUT_MS || 180000);
...
async function observeFinal(record, secret, conversationID, messageID) {
  ...
  try {
    ...
    return await readObserveStream(response, messageID, record);
  } finally {
    clearTimeout(timer);
  }
}

async function readObserveStream(response, messageID, record) {
  if (!response.body || !response.body.getReader) {
    return parseObserve(response.text ? await response.text() : "", messageID, record);
  }
  ...
  for (;;) {
    ...
    for (const frame of frames) {
      const result = parseObserveFrame(frame, messageID, record);
      if (result.done) return result.body;
    }
  }
  buffer += decoder.decode();
  if (buffer.trim()) {
    const result = parseObserveFrame(buffer, messageID, record);
    if (result.done) return result.body;
  }
  throw new Error(`observe ended before final event for ${messageID}`);
}

function parseObserve(text, messageID, record) {
...
function parseObserveFrame(frame, messageID, record = {}) {
  const event = {};
  ...
  if (event.event === "message" && data.type === "response" && data.from === record.gmail_agent && data.to === record.notifier_agent) {
    const error = responseBodyError(data.body);
    if (error) throw new Error(error);
    return { done: true, body: data.body || "" };
  }
  if (event.event !== "state_change" || data.message_id !== messageID) return { done: false };
  if (data.to_state === "error" && String(data.error || "").includes("ack timeout")) return { done: false };
  if (data.to_state === "error") throw new Error(data.error || responseBodyError(data.body) || "email message errored");
  if (data.to_state === "completed") return { done: true, body: data.body || "" };
  return { done: false };
}

function responseBodyError(body) {
  if (!body) return "";
  try {
...
function emailBody(record) {
  return [
    "Plain English",
    emailSummary(record) || defaultEmailSummary(record),
    "",
    "Details",
...
function emailSummary(record) {
  if (!record.email_summary_file || !fs.existsSync(record.email_summary_file)) return "";
  return redact(fs.readFileSync(record.email_summary_file, "utf8")).trim().slice(0, 4000);
}

function defaultEmailSummary(record) {
  if (record.status === "pass") {
    return `The background job "${record.name}" finished successfully. The detailed summary and logs are linked below.`;
  }
  return `The background job "${record.name}" did not finish cleanly. Check the summary and logs below before trusting any partial results.`;
}
