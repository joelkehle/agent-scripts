...
async function walkClaimFilesAsync(root, options = {}) {
  const readdir = options.readdir || fs.promises.readdir.bind(fs.promises);
  const entries = await readdir(root, { withFileTypes: true });
  const nested = await Promise.all(entries.map(async (entry) => {
    const file = path.join(root, entry.name);
    if (entry.isDirectory()) return walkClaimFilesAsync(file, options);
...
function readClaim(file, options = {}) {
  ...
  return classifyClaim(file, data, { claimsRoot, now });
}

function classifyClaim(file, data, options = {}) {
  const now = options.now || new Date();
  const claimsRoot = options.claimsRoot || path.dirname(file);
  const directory_repo = claimDirectoryIdentity(claimsRoot, file);
...
async function readClaimAsync(file, options = {}) {
  const now = options.now || new Date();
  const claimsRoot = options.claimsRoot || path.dirname(file);
  const directory_repo = claimDirectoryIdentity(claimsRoot, file);
...
async function readClaimsAsync(root, options = {}) {
  if (options instanceof Date) options = { now: options };
  const now = options.now || new Date();
  const claimsRoot = path.join(root, "claims");
  const concurrency = options.concurrency === undefined ? 32 : Number(options.concurrency);
  if (!Number.isInteger(concurrency) || concurrency <= 0) {
    throw new Error("claim read concurrency must be a positive integer");
  }
  let files;
  try {
    files = await walkClaimFilesAsync(claimsRoot, options);
  } catch (error) {
    if (error.code !== "ENOENT" || path.resolve(error.path || "") !== path.resolve(claimsRoot)) throw error;
    try {
      const stat = await fs.promises.stat(root);
      if (!stat.isDirectory()) throw new Error(`AgentCoord root is not a directory: ${root}`);
    } catch (rootError) {
      throw new Error(`AgentCoord root unavailable: ${rootError.message}`);
    }
    return [];
  }
  return mapWithConcurrency(files, concurrency, (file) => readClaimAsync(file, {
    claimsRoot,
    now,
