...
const MANIFEST_SCHEMA_V3 = "github-lifecycle-manifest.v3";
...
const DEFECT_CODES = {
  ...
  DOD_NON_GOALS_MISMATCH: "definition_of_done_non_goals_mismatch",
};
...
const WEEKLY_GOAL_FIELDS = {
  required: {
    schema: { type: "string", const: "agentcoord-weekly-goal-link.v1" },
    goal_id: { type: "string", pattern: ID_PATTERN },
    week_ending: { type: "string", pattern: DATE_PATTERN },
  },
  optional: {},
};

const MANIFEST_FIELDS_V3 = {
  required: {
    ...MANIFEST_FIELDS.required,
    weekly_goal: { type: "object" },
  },
  optional: MANIFEST_FIELDS.optional,
};
...
const DOD_KILL_FIELDS = {
  required: {
    ...
    decide_by: { type: "string", minLength: 1, pattern: "\\S" },
  },
  optional: {},
};

const DOD_DEFER_FIELDS = {
  required: {
    destination: { type: "string", minLength: 1, pattern: "\\S" },
    promotion_rule: { type: "string", minLength: 1, pattern: "\\S" },
  },
  optional: {},
};

const DOD_RATIFICATION_FIELDS = {
  required: {
    actor: { type: "string", const: "joelkehle" },
    evidence: { type: "string", const: "github_comment" },
  },
  optional: {},
};
...
const DOD_FIELDS = {
  required: {
    class: { type: "string", enum: ["full", "lightweight"] },
    ...
    defer_policy: { type: "object" },
    ratification: { type: "object" },
...
function buildJsonSchema(schema = MANIFEST_SCHEMA) {
  if (![MANIFEST_SCHEMA, MANIFEST_SCHEMA_V2, MANIFEST_SCHEMA_V3].includes(schema)) {
  ...
  if ([MANIFEST_SCHEMA_V2, MANIFEST_SCHEMA_V3].includes(schema)) {
    ...
    dod.defer_policy = jsonSchemaObject(DOD_DEFER_FIELDS, "Deferral destination and promotion rule");
    dod.ratification = jsonSchemaObject(DOD_RATIFICATION_FIELDS, "Human ratification evidence policy");
    dod.finding_policy = jsonSchemaObject(DOD_POLICY_FIELDS, "Finding disposition policy");
  }

  if (schema === MANIFEST_SCHEMA_V3) {
    root.required.push("weekly_goal");
    root.properties.weekly_goal = jsonSchemaObject(WEEKLY_GOAL_FIELDS, "Weekly goal link");
  }
