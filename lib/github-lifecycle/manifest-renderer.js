...
function renderBodyContent(manifest, issue) {
  ...
  if (issue.definition_of_done) {
    ...
    lines.push("");
    if (dod.non_goals.length === 0) {
      lines.push("- None declared");
    }
    for (const nonGoal of dod.non_goals) {
      lines.push(`- ${nonGoal}`);
    }
    lines.push("");
    ...
    lines.push(
      "- Claiming requires a GitHub comment by that actor carrying a " +
        "`ratified-definition-of-done.v1` marker for the opening marker's exact `payload_sha256`.",
    );
  }
...
function renderIssue(manifest, issueId) {
  ...
  const spec = manifest.specification;
  const marker =
    `<!-- ${manifest.schema} spec=${spec.spec_id} revision=${spec.source_revision}` +
