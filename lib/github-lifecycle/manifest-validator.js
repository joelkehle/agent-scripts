...
function checkDodText(value, path, errors) {
  if (typeof value === "string" && (value.trim() === "" || VAGUE_DOD_TEXT.test(value.trim()))) {
...
function checkDefinitionOfDone(issue, path, errors) {
  ...
  if (Array.isArray(dod.proof_requirements)) {
    dod.proof_requirements.forEach((proof, index) => {
      ...
      if (typeof proof.id === "string") {
        if (proofs.has(proof.id)) {
          defect(
            errors,
            DEFECT_CODES.DUPLICATE_DOD_PROOF_ID,
            `${proofPath}.id`,
            `duplicates proof requirement ${proof.id}`,
          );
        }
        proofs.add(proof.id);
      }
  ...
  if (Array.isArray(dod.kill_criteria)) {
    dod.kill_criteria.forEach((criterion, index) => {
      ...
      checkDodText(criterion.decide_by, `${criterionPath}.decide_by`, errors);
    });
  }

  if (Array.isArray(dod.non_goals)) {
    dod.non_goals.forEach((nonGoal, index) => {
      checkDodText(nonGoal, `${dodPath}.non_goals[${index}]`, errors);
    });
    if (Array.isArray(issue.non_goals) && JSON.stringify(dod.non_goals) !== JSON.stringify(issue.non_goals)) {
      defect(errors, DEFECT_CODES.DOD_NON_GOALS_MISMATCH, `${dodPath}.non_goals`, "must exactly match the issue non_goals list");
    }
  }

  if (dod.defer_policy && typeof dod.defer_policy === "object" && !Array.isArray(dod.defer_policy)) {
    checkObject(dod.defer_policy, DOD_DEFER_FIELDS, `${dodPath}.defer_policy`, errors);
    checkDodText(dod.defer_policy.destination, `${dodPath}.defer_policy.destination`, errors);
    checkDodText(dod.defer_policy.promotion_rule, `${dodPath}.defer_policy.promotion_rule`, errors);
  }

  if (dod.ratification && typeof dod.ratification === "object" && !Array.isArray(dod.ratification)) {
    checkObject(dod.ratification, DOD_RATIFICATION_FIELDS, `${dodPath}.ratification`, errors);
  }
...
function collectIssues(manifest, declaredRequirements, errors) {
  ...
  manifest.issues.forEach((issue, index) => {
    ...
    const structured = [MANIFEST_SCHEMA_V2, MANIFEST_SCHEMA_V3].includes(manifest.schema);
    checkObject(issue, structured ? ISSUE_FIELDS_V2 : ISSUE_FIELDS, path, errors);
    if (structured) {
...
function validateManifest(manifest) {
  ...
  checkObject(manifest, manifest.schema === MANIFEST_SCHEMA_V3 ? MANIFEST_FIELDS_V3 : MANIFEST_FIELDS, "", errors);
  if ([MANIFEST_SCHEMA_V2, MANIFEST_SCHEMA_V3].includes(manifest.schema)) {
    errors.splice(
      0,
      errors.length,
  ...
  if (manifest.schema === MANIFEST_SCHEMA_V3) {
    if (manifest.weekly_goal && typeof manifest.weekly_goal === "object" && !Array.isArray(manifest.weekly_goal)) {
      checkObject(manifest.weekly_goal, WEEKLY_GOAL_FIELDS, "weekly_goal", errors);
    }
  }
  ...
  for (const error of errors) {
    if (
      [MANIFEST_SCHEMA_V2, MANIFEST_SCHEMA_V3].includes(manifest.schema) &&
  ...
  return {
    schema: VALIDATION_SCHEMA,
    manifest_schema: [MANIFEST_SCHEMA, MANIFEST_SCHEMA_V2, MANIFEST_SCHEMA_V3].includes(manifest.schema) ? manifest.schema : MANIFEST_SCHEMA,
    ok: errors.length === 0,
    errors,
