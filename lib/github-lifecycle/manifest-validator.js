...
function checkDefinitionOfDone(issue, path, errors) {
  ...
  if (Array.isArray(dod.non_goals)) {
    ...
    if (
      Array.isArray(issue.non_goals) &&
      JSON.stringify(dod.non_goals) !== JSON.stringify(issue.non_goals)
    ) {
      defect(
        errors,
        DEFECT_CODES.DOD_NON_GOALS_MISMATCH,
        `${dodPath}.non_goals`,
        "must exactly match the issue non_goals list",
      );
    }
  }
