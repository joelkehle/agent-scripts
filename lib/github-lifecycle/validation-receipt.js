...
function buildValidationReceipt(options) {
  ...
  const receipt = {
    ...
    subject: {
      ...
      ...(manifest && ["github-lifecycle-manifest.v2", "github-lifecycle-manifest.v3"].includes(manifest.schema)
        ? {
        ...
        : {}),
      ...(manifest && manifest.schema === "github-lifecycle-manifest.v3"
        ? { weekly_goal: manifest.weekly_goal || null }
        : {}),
    },
    input: {
