...
const FOCUS_SCHEMA_V2 = "agentcoord-weekly-focus.v2";
const LEGACY_EXECUTION_REFS_FIELD = "execution_refs";
const V2_EXECUTION_FIELDS = new Set([
...
const SUPERVISED_EXECUTION_FIELDS = new Set(["required"]);
...
const COMPACT_GOAL_LINE_MAX = 100;
const COMPACT_GOAL_ID_MAX = COMPACT_GOAL_LINE_MAX - ": ".length - "...".length;
...
function compactGoalLine(goal) {
  const prefix = `${goal.id}: `;
  const text = goal.done.replace(/\s+/g, " ").trim();
  const available = COMPACT_GOAL_LINE_MAX - Array.from(prefix).length;
  const characters = Array.from(text);
  if (characters.length <= available) return `${prefix}${text}`;
  return `${prefix}${characters.slice(0, available - 3).join("").trimEnd()}...`;
}

function formatFocusList(focus, options = {}) {
  const lines = [focus.expired
    ? `Weekly focus EXPIRED after ${focus.week_ending} in ${focus.time_zone}; current date ${focus.current_date}`
    : `Weekly focus through ${focus.week_ending} in ${focus.time_zone}`];
  if (!options.full) {
    for (const goal of focus.goals) lines.push(compactGoalLine(goal));
    if (focus.not_this_week.length) {
      const count = focus.not_this_week.length;
      lines.push(`Not this week: ${count} ${count === 1 ? "item" : "items"} (use --full to expand)`);
    }
    return lines.join("\n");
  }
  for (const goal of focus.goals) {
    lines.push(goal.id);
    lines.push(`  done: ${goal.done}`);
    lines.push(`  required milestone: ${goal.required_milestone}`);
    lines.push(`  fallback: ${goal.fallback}`);
    for (const ref of goal.execution_refs || []) {
      lines.push(`  legacy execution: ${ref.kind}:${ref.id}`);
    }
    if (goal.active_execution_ref) {
      lines.push(`  active execution: ${goal.active_execution_ref.kind}:${goal.active_execution_ref.id}`);
    }
    for (const ref of goal.proof_execution_refs || []) {
      lines.push(`  proof execution: ${ref.kind}:${ref.id}`);
    }
    if (goal.supervised_execution) {
      lines.push(`  supervision required: ${goal.supervised_execution.required}`);
    }
  }
  if (focus.not_this_week.length) {
    lines.push("Not this week:");
    for (const item of focus.not_this_week) lines.push(`- ${item}`);
  }
  return lines.join("\n");
}
  ...
  for (let index = 0; index < lines.length; ) {
    ...
    if (section === "goals") {
      ...
      if (indent === 2 && newGoal) {
        ...
        indent === 6 &&
        newGoal &&
        [LEGACY_EXECUTION_REFS_FIELD, "proof_execution_refs"].includes(nestedField) &&
        Array.isArray(goal?.[nestedField])
      ) {
        executionRef = {};
        goal[nestedField].push(executionRef);
        ...
        index += 1;
        continue;
      } else if (indent === 6 && field && nestedField === "active_execution_ref") {
        const [, key, remainder] = field;
        assign(
          goal.active_execution_ref,
          key,
          scalar(remainder, `line ${lineNumber}`, errors),
          `line ${lineNumber}`,
          EXECUTION_REF_FIELDS,
        ...
        index += 1;
        continue;
      } else if (indent === 6 && field && nestedField === "supervised_execution") {
        const [, key, remainder] = field;
        assign(
          goal.supervised_execution,
          key,
          scalar(remainder, `line ${lineNumber}`, errors),
          `line ${lineNumber}`,
          SUPERVISED_EXECUTION_FIELDS,
          errors,
        );
        index += 1;
        continue;
      ...
      const [, key, remainder] = match;
      const rawValue = remainder.trim();
      if ([LEGACY_EXECUTION_REFS_FIELD, "proof_execution_refs"].includes(key)) {
        if (rawValue && rawValue !== "[]") {
          errors.push(`line ${lineNumber}: ${key} must be a block list or []`);
        }
        assign(goal, key, [], `line ${lineNumber}`, GOAL_FIELDS, errors);
        executionRef = null;
        nestedField = rawValue === "" ? key : "";
        index += 1;
      } else if (key === "active_execution_ref") {
        if (rawValue) errors.push(`line ${lineNumber}: active_execution_ref must be a block object`);
        assign(goal, key, {}, `line ${lineNumber}`, GOAL_FIELDS, errors);
        executionRef = null;
        nestedField = rawValue === "" ? key : "";
        index += 1;
      } else if (key === "supervised_execution") {
        if (rawValue) errors.push(`line ${lineNumber}: supervised_execution must be a block object`);
        assign(goal, key, {}, `line ${lineNumber}`, GOAL_FIELDS, errors);
        executionRef = null;
        nestedField = rawValue === "" ? key : "";
        index += 1;
  ...
  if (!Array.isArray(focus.goals) || focus.goals.length === 0) {
    ...
    focus.goals.forEach((item, index) => {
      ...
      if (typeof item.id === "string" && item.id.trim()) {
        ...
        if (/[\r\n\u2028\u2029]/u.test(item.id)) {
          errors.push(`goals[${index}].id must not contain line breaks`);
        }
        if (Array.from(item.id).length > COMPACT_GOAL_ID_MAX) {
          errors.push(`goals[${index}].id must be at most ${COMPACT_GOAL_ID_MAX} characters`);
        }
      ...
      const v2Fields = [...V2_EXECUTION_FIELDS].filter((field) => Object.hasOwn(item, field));
      if (isV2 && hasLegacyRefs) {
        errors.push(`goals[${index}] mixes ${FOCUS_SCHEMA_V2} with legacy execution_refs`);
      }
      if (!isV2 && v2Fields.length) {
        errors.push(`goals[${index}] uses version 2 execution fields without schema ${FOCUS_SCHEMA_V2}`);
      }
      if (hasLegacyRefs && v2Fields.length) {
        errors.push(`goals[${index}] must not mix execution_refs with version 2 execution fields`);
      }
      ...
      if (Object.hasOwn(item, "supervised_execution")) {
        if (!item.supervised_execution || typeof item.supervised_execution !== "object" || Array.isArray(item.supervised_execution)) {
          errors.push(`goals[${index}].supervised_execution must be an object`);
        } else if (typeof item.supervised_execution.required !== "boolean") {
          errors.push(`goals[${index}].supervised_execution.required must be a Boolean`);
        }
      }

      const seenRefs = new Map();
      if (Object.hasOwn(item, "active_execution_ref")) {
        const location = `goals[${index}].active_execution_ref`;
        const refErrors = validateExecutionRef(item.active_execution_ref, location);
        errors.push(...refErrors);
        if (!refErrors.length) seenRefs.set(`${item.active_execution_ref.kind}\0${item.active_execution_ref.id}`, location);
      }
      if (Object.hasOwn(item, "proof_execution_refs")) {
        if (!Array.isArray(item.proof_execution_refs)) {
          errors.push(`goals[${index}].proof_execution_refs must be a list`);
        } else {
          item.proof_execution_refs.forEach((ref, refIndex) => {
            const location = `goals[${index}].proof_execution_refs[${refIndex}]`;
            const refErrors = validateExecutionRef(ref, location);
            errors.push(...refErrors);
            if (refErrors.length) return;
            const key = `${ref.kind}\0${ref.id}`;
            if (seenRefs.has(key)) errors.push(`${location} duplicates ${seenRefs.get(key)}`);
            else seenRefs.set(key, location);
          });
        }
      }
    });
