...
function compactGoalLine(goal) {
  const line = `${goal.id}: ${goal.done.replace(/\s+/g, " ").trim()}`;
  const characters = Array.from(line);
  if (characters.length <= COMPACT_GOAL_LINE_MAX) return line;
  return `${characters.slice(0, COMPACT_GOAL_LINE_MAX - 3).join("").trimEnd()}...`;
}
