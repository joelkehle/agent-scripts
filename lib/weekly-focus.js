  ...
  for (let index = 0; index < lines.length; ) {
    ...
    if (section === "goals") {
      ...
      if (key === "execution_refs") {
        if (rawValue) errors.push(`line ${lineNumber}: execution_refs must be a block list`);
...
function loadFocus(file = DEFAULT_FOCUS_FILE) {
  let text;
  try {
  ...
  return { ...result.focus, file };
}

function resolveGoal(focus, goalId) {
...
function resolveException(focus, category, reason) {
