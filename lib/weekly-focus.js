...
const COMPACT_DONE_RULE_MAX = 100;
...
function compactDoneRule(done) {
  const text = done.replace(/\s+/g, " ").trim();
  const characters = Array.from(text);
  if (characters.length <= COMPACT_DONE_RULE_MAX) return text;
  return `${characters.slice(0, COMPACT_DONE_RULE_MAX - 3).join("").trimEnd()}...`;
}

function compactGoalLine(goal) {
  return `${goal.id}: ${compactDoneRule(goal.done)}`;
}
