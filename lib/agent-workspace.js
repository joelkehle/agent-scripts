...
function beginRun(options) {
  ...
  const sessionKind = preflight.observation.git ? "repository" : "workspace";
  if (sessionKind === "workspace" && !focus.goal_id) {
    throw new WorkspaceRunError("workspace operator sessions require --goal");
  }
