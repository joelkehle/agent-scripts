...
const ALL_STATES = new Set(["starting", "active", "sealed", "quarantined", "abandoned"]);
...
function readManifest(file) {
  ...
  try {
    ...
    throw new WorkspaceRunError(`cannot read manifest ${file}: ${error.message}`);
  }
  if (manifest.schema !== "agent-workspace-run.v1") {
    throw new WorkspaceRunError(`unsupported manifest schema in ${file}`);
  }
  if (!ALL_STATES.has(manifest.state)) {
    throw new WorkspaceRunError(`invalid manifest state in ${file}`);
  }
  return manifest;
}
...
function resolveFocus(options) {
  ...
  if (options.goalId) {
    return { resolution: resolveGoal(focus, options.goalId), goal_id: options.goalId, exception: null };
  }
  if (options.exceptionCategory || options.exceptionReason) {
    const resolution = resolveException(focus, options.exceptionCategory, options.exceptionReason);
...
function beginRun(options) {
  ...
  const base = {
    ...
    git_common_dir: preflight.observation.git_common_dir,
    canonical_remote: preflight.canonical_remote,
  ...
  return { file, manifest: active, focus: focus.resolution, preflight: publicPreflight(preflight) };
}
...
function reconcileRuns(options = {}) {
  ...
  for (const entry of readManifests(stateRoot)) {
    if (!entry.manifest || !ACTIVE_STATES.has(entry.manifest.state)) continue;
