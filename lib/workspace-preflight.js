...
function repositoryAliases(root, remote) {
  const aliases = new Set([path.resolve(root).toLowerCase()]);
  ...
  for (const file of walkFiles(path.join(root, "claims"), ".json")) {
    try {
      ...
      if (!claim.released_at && claim.safety === "write" && Number.isFinite(expires) && expires >= now.getTime()) {
        const sameRepository = repositoryMatches(claim.repo, aliases);
        const scopeOverlap = sameRepository && Array.isArray(claim.scope) && claim.scope.some((scope) => {
        ...
        if (sameRepository && scopeOverlap) active.push({ file, claim });
      }
    } catch (error) {
...
const RUN_STATES = new Set(["starting", "active", "sealed", "quarantined", "abandoned"]);
...
function validateRunManifest(manifest) {
  ...
  if (typeof manifest.branch !== "string" || !manifest.branch.trim()) errors.push("branch must be a non-empty string");
  if (!SHA_PATTERN.test(String(manifest.starting_head || ""))) errors.push("starting_head must be a Git object ID");
  ...
  if (manifest.ending_head !== null && !SHA_PATTERN.test(String(manifest.ending_head || ""))) {
  ...
  if (ACTIVE_RUN_STATES.has(manifest.state)) {
    ...
    if (manifest.ending_head !== null) errors.push("active run ending_head must be null");
  }
  ...
  } else if (manifest.quarantine_reason !== null) {
    errors.push("quarantine_reason must be null unless state is quarantined");
  }
  return errors;
}
...
function evaluateWorkspace(observation, mode = "write") {
  ...
  if (!observation.git) {
    addIssue(issue("not_git_repository", "supplied root is not a Git repository"), true, true);
