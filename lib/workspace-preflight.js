...
function evaluateWorkspace(observation, mode = "write") {
  ...
  if (!observation.git) {
    ...
    addIssue(issue("workspace_operator_session", "non-Git directory permits a read-only operator session"), false, false);
...
function preflightWorkspace(root, mode = "write", options = {}) {
  ...
  return {
    ...
    session_kind: observation.git ? "repository" : (observation.path_exists && observation.is_directory ? "workspace" : null),
