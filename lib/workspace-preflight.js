...
const SOURCE_WRITE_OWNER_HOST = "dev";

function normalizeHost(value) {
  return String(value || "").trim().toLowerCase().split(".")[0];
}

function sourceWriteOwnership(currentHost = os.hostname()) {
  const current = normalizeHost(currentHost);
  return {
    schema: "source-write-ownership.v1",
    owner_host: SOURCE_WRITE_OWNER_HOST,
    current_host: current || null,
    write_allowed: current === SOURCE_WRITE_OWNER_HOST,
...
function validateRunManifest(manifest) {
  ...
  // Routine runs (Joel-ratified 2026-08) carry neither a goal nor an
  // exception; a manifest may still never carry both.
  if (hasGoal && hasException) errors.push("manifest must not contain both goal_id and exception");
  if (hasException) {
...
function collectWorkspace(root, options = {}) {
  const sourceOwnership = sourceWriteOwnership(options.currentHost);
  const localSourceWrite = options.mode === "write" && options.sessionKind !== "workspace";
  ...
  const claims = localSourceWrite
    ? { available: false, active: [], invalid: [], skipped: true, reason: "source writes use local workspace ownership" }
    : Array.isArray(options.claimEntries)
  ...
  return {
    ...
    worktrees,
    claims,
    quarantine: localSourceWrite
      ? { available: false, matches: [], skipped: true, reason: "source writes use local workspace ownership" }
      : quarantineMatch(quarantineRoot, aliases),
...
function evaluateWorkspace(observation, mode = "write", sessionKind = "repository") {
  ...
  const sourceOwnership = observation.source_ownership || sourceWriteOwnership();
  if (mode === "write" && !sourceOwnership.write_allowed) {
    addIssue(issue(
      "source_write_host_refused",
      `source writes belong to ${sourceOwnership.owner_host}; current host is ${sourceOwnership.current_host || "unknown"}`,
      {
        owner_host: sourceOwnership.owner_host,
        current_host: sourceOwnership.current_host,
      },
    ));
  }
  ...
  if (!observation.quarantine.available && !observation.quarantine.skipped) {
  ...
  if (!observation.claims.available && !observation.claims.skipped) {
...
function preflightWorkspace(root, mode = "write", options = {}) {
  ...
  const observation = collectWorkspace(root, { ...options, mode, sessionKind });
  ...
  return {
    ...
    source_ownership: observation.source_ownership || sourceWriteOwnership(options.currentHost),
