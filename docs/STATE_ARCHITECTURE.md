...
| Agreement templates | Box — owned by TDG Contracts (Joel, 2026-08-22). Agents have NO Box access by design: the only path would be Joel's own account and it cannot be made read-only, so the system records pointers and handling notes ("attach from the Box OFRAS folder") and a human fetches the file | term sheets, license/option/NDA agreement bodies, the IP-DD form | git and local files hold pointers, requirements, fixtures, or clearly labeled examples only |
| Other templates and working documents | the work document store — Google Drive (Joel, 2026-08-22); agents may read through Joel's connected tools where granted | non-agreement templates, disclosure-instruction attachments, working documents | git holds pointers, requirements, fixtures, or labeled examples only |
