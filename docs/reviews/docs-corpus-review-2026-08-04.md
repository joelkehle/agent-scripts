...
This is the front door for every coding session, and it assumes you already live here. The startup-packet list (lines 25-41) names nine tools with zero definitions: "`wwi` open loops", "AgentCoord validation plus active and stale claims", "machine-compliance", "docs-list output". Line 63 tells the reader to run `yolo resume` — `yolo` is defined nowhere in the corpus (it appears again in `manager/docs/decisions/fleet-version-rollout-plan.md:91` as "the current `yolo` alias runs Codex..."). The page also mixes three concerns: the launch ritual, Codex thread-naming etiquette, and installer internals (symlink vs hard link vs copy).
...
6. **Rewrite `agent-start.md` as a real front door.** Goal: "Give `agent-scripts/docs/agent-start.md` a summary paragraph, a one-line definition (or link) for every item in the startup packet (wwi, AgentCoord, machine-compliance, bus-discover, docs-list, agent-check, yolo), and move Codex thread-naming and installer internals to their own pages."
